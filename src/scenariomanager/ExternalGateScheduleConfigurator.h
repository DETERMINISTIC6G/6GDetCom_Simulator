...
class ExternalGateScheduleConfigurator
    : public TSNschedGateScheduleConfigurator {
...
protected:
  ...
  virtual Output *convertJsonToOutput(const Input &input,
                                      const cValueMap *json) const override;
  virtual void printJson(std::ostream &stream, const cValue &value,
                         int level = 0) const;

  /*ExternalGateScheduleConfigurator*/
  virtual bool isDetComLink(cModule *source, cModule *target,
...
private:
  ...
  bool addEntryToPDBMap(cValueArray *pdb_map, cModule *source,
