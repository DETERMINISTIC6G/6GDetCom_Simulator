...
void ChangeMonitor::initialize(int stage) {
  if (stage == INITSTAGE_LOCAL) {
    ...
    gateScheduleConfigurator =
        check_and_cast<GateScheduleConfiguratorBase *>(getModuleByPath(path));
  ...
  for (int i = 0; i < sim->getLastComponentId(); ++i) {
    ...
    if (mod) {
      if (!strcmp(mod->getModuleType()->getName(), "ObservedScenarioManager")) {
        EV << "Subscribing module: " << mod->getFullPath()
           << " to the observer." << endl;
      ...
      if (!strcmp(mod->getModuleType()->getName(), "TsnTranslator") &&
          mod->par("isDstt")) { // subscribe only DSTT
        EV << "Subscribing module: " << mod->getFullPath()
           << " to the observer." << endl;
        mod->subscribe(DynamicScenarioObserver::distributionChangeSignal,
                       observer);
      }
    } // endif
  } // endfor
...
void ChangeMonitor::prepareChangesForProcessing(int initialized) {
  ...
  for (const auto &mapping : streamConfigurations) {
    intervals.push_back(
        std::round(mapping.packetInterval.doubleValueInUnit("ns")));
  }
...
void ChangeMonitor::configureInitStreamsAndDistributions() {
  ...
  if (mappingParameter->size()) {
    ...
    for (int i = 0; i < mappingParameter->size(); i++) {
      auto element =
          check_and_cast<cValueMap *>(mappingParameter->get(i).objectValue());
    ...
    for (int i = 0; i < sim->getLastComponentId(); ++i) {
      ...
      if (mod &&
          !strcmp(mod->getNedTypeName(),
                  "d6g.apps.dynamicsource.DynamicPacketSource")) { // app source
                                                                   // module
        ...
        sourceModule->flowName =
            streamConfigurations[sourceNumber].name; // set unique flow name
        sourceNumber++;
        delete element;
      } else if (mod &&
                 !strcmp(mod->getNedTypeName(),
                         "d6g.devices.tsntranslator.TsnTranslator") &&
                 mod->par("isDstt")) { // DSTT translator
...
void ChangeMonitor::addEntriesToDistributionsFor(
    TsnTranslator *translator) { // both: Uplink and Downlink
  auto delayParam = {"Uplink", "Downlink"};
  for (std::string param : delayParam) {
    auto dynExpr =
        translator->getDistributionExpression(("delay" + param).c_str());
    auto element = observer->createHistogram(*dynExpr);
    take(element);
    drop(element);
    auto bridgePort = std::string(translator->getParentModule()->getName()) +
...
void ChangeMonitor::addEntryToStreamConfigurations(cValueMap *element, int i) {
  ...
  mapping.gateIndex = // element->get("gateIndex").intValue();
      element->containsKey("gateIndex") ? element->get("gateIndex").intValue()
...
int ChangeMonitor::classify(int pcp) {
  ...
  if (pcp < 0 || pcp > pcpMapping->size() - 1)
    return 0;
  auto pcpToGateIndex =
      check_and_cast<cValueArray *>(pcpMapping->get(pcp).objectValue());
